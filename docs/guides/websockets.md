...
- **Authorization has to live in the handler's `canHandle`.** The analytics
...
sensitive still authorize in `canHandle`, which is what the analytics plugin
does.
...
it, and do the authorization there: `req` is the full upgrade request, headers
...
| `/_analytics_ws` | `AnalyticsWSHandler` | analytics plugin; authorizes in `canHandle` |
