...
specialized handlers sit above the fallback to transform particular URL shapes.
...
1. The resolved path must be inside the root (a prefix test, after normalizing
   `..`).
...
`config.importMap` adds to or overrides that map. Relative values are normalized
to absolute URLs.
