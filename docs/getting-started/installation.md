...
its posts in memory instead, and `@bakery-framework/orm` is not installed at all: the CLI treats it as an optional peer, so an app that does not want a database
...
> re-serializes the file as plain JSON: losing comments and formatting
