 ...
 *
 * It exists because four handlers were each spelling that out themselves ( * StaticHandler, PublicHandler, ImageHandler and NMHandler) in four slightly
...
export async function getStatic(
  ...
  for (const candidate of candidates) {
    ...
    // escaped the root, and isForbidden additionally honors `.forbidden`
    ...
    // caller reaching for `getStatic` as a one-stop authorization check is
