 ...
 * not, so `@bakery/core/plugins` pointed straight at `routes.ts` — which meant
 * `definePlugin` and `ServerPlugin` were only reachable through
 * `@bakery/core/plugins/types`, a second subpath for one concept. It also broke
 * under TypeScript's `paths` resolution, which maps `@bakery/core/plugins` to
